...
def run_init_experiment():
    ...
    print(f"{'Init Type':<15} | {'Train Acc':<10} | {'Test Acc':<10}")
    print("-" * 40)
    
    for init in init_types:
        model = Perceptron(init_type=init)
        model.fit(
            X_train_scaled, 
            y_train, 
            X_test_scaled, 
...
def plot_init_results(histories):
    n = len(histories)
    fig, axes = plt.subplots(1, n + 1, figsize=(4 * (n + 1), 5))
    fig.suptitle("Влияние инициализации весов", fontsize=16, y=1.05)
    ...
    plt.savefig("report/experiment_plots/weights_init_exp.jpg", bbox_inches='tight', dpi=300)
    plt.show()
