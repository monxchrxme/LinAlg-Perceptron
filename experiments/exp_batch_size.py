...
def run_batch_size_experiment():
    ...
    print(f"{'Batch Size':<10} | {'Train Acc':<10} | {'Test Acc':<10}")
    print("-" * 35)
...
def plot_batch_results(histories):
    n = len(histories)
    fig, axes = plt.subplots(1, n + 1, figsize=(4 * (n + 1), 5))
    fig.suptitle("Влияние размера батча (Batch Size)", fontsize=16, y=1.05)
    ...
    plt.savefig("report/experiment_plots/batch_size_exp.jpg", bbox_inches='tight', dpi=300)
    plt.show()
